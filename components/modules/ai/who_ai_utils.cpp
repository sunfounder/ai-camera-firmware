...
void get_detection_result(std::list<dl::detect::result_t> &results, uint8_t *output) {
    // int* result = (int*)malloc(results.size() * sizeof(int) * 14);
    // int i = 0;
    // int j = 0;
    // for (std::list<dl::detect::result_t>::iterator prediction = results.begin(); prediction != results.end(); prediction++, i++) {
    //     if (prediction->keypoint.size() != 10) return NULL;
    //     // ESP_LOGI("detection_result", "[%2d]: (%3d, %3d, %3d, %3d)", i, prediction->box[0], prediction->box[1], prediction->box[2], prediction->box[3]);
    //     // ESP_LOGI("detection_result", "      left eye: (%3d, %3d), right eye: (%3d, %3d), nose: (%3d, %3d), mouth left: (%3d, %3d), mouth right: (%3d, %3d)",
    //     //             prediction->keypoint[0], prediction->keypoint[1],  // left eye
    //     //             prediction->keypoint[6], prediction->keypoint[7],  // right eye
    ...
    //     //             prediction->keypoint[2], prediction->keypoint[3],  // mouth left corner
    //     //             prediction->keypoint[8], prediction->keypoint[9]); // mouth right corner
    //     for (j=0; j<prediction->box.size(); j++) {
    //         result[i*14+j] = prediction->box[j];
    //     }
    //     for (j=0; j<prediction->keypoint.size(); j++) {
    //         result[i*14+j+4] = prediction->keypoint[j];
    //     }
    // }

    // std::list<dl::detect::result_t>::iterator prediction = results.begin();
    // if (prediction->keypoint.size() != 10) return;
    // output[0] = prediction->keypoint[4];
    // output[1] = prediction->keypoint[5];

    int i = 0;
    std::list<dl::detect::result_t>::iterator prediction = results.begin();
    if (prediction->keypoint.size() != 10) return;
    for (i=0; i<prediction->box.size(); i++) {
        output[i] = prediction->box[i];
    }
    for (i=0; i<prediction->keypoint.size(); i++) {
        output[i+4] = prediction->keypoint[i];
    }
}
